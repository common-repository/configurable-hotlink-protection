...
Description: Save bandwidth by easily blocking links to video, audio, and other files from unapproved 3rd-party sites. Requires mod_rewrite.<br /><br />Please direct any support questions to <a href="http://wordpress.org/tags/configurable-hotlink-protection?forum_id=10">the WordPress forums</a>.
Version: 0.9
Author: Jeff Brand
...
class DFHotlink {
	...
	public static function admin_init() {
		...
		register_setting( 'df_hotlink', 'hotlink_usecontentplaceholders', 'intval' );
		register_setting( 'df_hotlink', 'hotlink_contentplaceholderstorage', array( __CLASS__, 'sanitize_contentplaceholderstorage' ) );
	...
	public static function sanitize_contentplaceholderstorage( $val ) {
		if ( $val ) $val = 'wp-content/hotlink-placeholders';
		return $val;
	}
...
// Placeholder rule:
//	RewriteRule \.(mp3)$ /wp-content/plugins/configurable-hotlink-protection/placeholder.php [NC,L]
	...
	public static function contentplaceholder_storage_check() {
		if ( ! get_option('hotlink_usecontentplaceholders') )
			return;

		$path = sanitize_contentplaceholderstorage( get_option('hotlink_contentplaceholderstorage') );
		if ( ! file_exists($path) ) @mkdir($path, 755);

		if ( ! file_exists( $path ) )
			echo '<div class="error"><p>The content placeholder feature is enabled but the storage folder could not be created.</p></div>';
	}

	public static function activate() {
		self::contentplaceholder_storage_check();
